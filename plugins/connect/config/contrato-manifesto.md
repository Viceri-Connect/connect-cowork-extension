...
> Mecanismo, entregue e mantido pelo **produto** (corte `_`/conteúdo) — o Connect
...
## 1. Manifesto × acervo — o mesmo conceito em todos os níveis
...
campo `onedrive-rel` carregava: não existe mais path nenhum pra formatar errado no
coletivo.
...
- **Registro autorado é proibido.** Não existe `_cerebro/sub-vaults.json` nem índice de
  entidades escrito à mão. Reintroduziria a duplicação que o modelo recusa (ver
  `manifesto-e-acervo` no GLOSSARIO.md).
...
- **Render/L1 nunca expande acervo:** o bloco de sessão carrega manifesto + ponteiros,
  nunca o conteúdo do acervo — o custo de token fica plano por mais que o grafo cresça.
- **Wikilink cross-acervo é ponteiro tipado resolve-on-touch:** link para conteúdo num
...
## 4. O grafo de dependências — arestas bidirecionais
...
- **Relação declarada, nunca hardcoded.** `cliente` é tipo filho de `tribo` por convenção da
  instância, não por prescrição do produto ("nada engessado").
